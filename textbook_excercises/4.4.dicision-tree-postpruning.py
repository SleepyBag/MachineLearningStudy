...
class Node:
    # 决策树的结点
    def __init__(self, data_frame=None, label_column=None):
        self.child = {}
...
def build_decision_node(root, father, split_value, node, train_data_frame, test_data_frame, discrete_columns, continuous_columns, label_column, pruning=None):
    print(train_data_frame[discrete_columns +
...
def post_prune(root, father, split_value, node, test_data_frame, label_column):
    # 后剪枝过程
    ...
    for key in node.child.keys():
        child_node = node.child[key]
        # 如果有一个子结点不是叶结点,那么当前结点就不能剪枝
        is_leaf = post_prune(root, node, key, child_node,
                             test_data_frame, label_column)
    ...
    if is_leaf:
        ...
        # 计算剪枝后的精度
        pruned_node = Node()
        pruned_node.label = node.label
        if root != node:
            father.child[split_value] = pruned_node
            pruned_accuracy = test(root, test_data_frame, label_column)
            print('pruned_accuracy', pruned_accuracy)

            # 若不剪枝比较好,那么把父亲结点的子结点换为原结点
            if cur_accuracy > pruned_accuracy:
                father.child[split_value] = node
                return False
            else:
                return True
        else:
            pruned_accuracy = test(pruned_node, test_data_frame, label_column)
            print('pruned_accuracy', pruned_accuracy)
...
def build_decision_tree(train_data_frame, test_data_frame, discrete_columns, continuous_columns, label_column, pruning=None):
    ...
    if pruning == 'post':
        root = post_prune(root, None, None, root,
                          test_data_frame, label_column)
    return root
...
root = build_decision_tree(train_data_frame, test_data_frame, discrete_columns=discrete_columns,
                           continuous_columns=continuous_columns, label_column=label_column, pruning='post')
